-- notify_customer_order_change's status CASE has no branch for 'pending' at
-- all, so it silently falls to ELSE (no alert, no email) whenever an order's
-- order_status becomes 'pending' - which is exactly the status
-- confirmCheckoutOrder.ts sets on a REGULAR card-checkout order the moment
-- its payment is confirmed (pay-for-me links land on 'in_review' instead,
-- which IS handled). Confirmed against real order history: two paystack
-- orders show alerts for "Order confirmed" and "Order processing" but
-- nothing at all for the payment-confirmation moment they passed through
-- 'pending' in between. Add the missing branch, guarded the same way as
-- 'in_review' so it only fires once payment is genuinely confirmed.
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    IF NEW.order_status IS DISTINCT FROM OLD.order_status THEN
      ...
      CASE NEW.order_status
        ...
        WHEN 'pending'    THEN
          IF NEW.payment_status = 'paid' THEN
            v_title := 'Payment received'; v_message := 'Your payment was received. Your order is now being processed.';
          ELSE
            v_title := NULL; v_should_email := false;
          END IF;
