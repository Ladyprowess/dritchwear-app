...
// Renders in a fully isolated document (iframe on web, WebView on native)
// rather than injecting into the surrounding React tree via
// dangerouslySetInnerHTML - the same technique PaystackPayment.tsx already
// uses for the same reason: arbitrary HTML rendered inline can be affected
// by the host app's own global styles/reset, and on native there's no
// innerHTML equivalent at all. An isolated document sidesteps both.
function HtmlPreview({ html }: { html: string }) {
  const doc = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{margin:0;padding:0;font-family:Arial,sans-serif;font-size:14px;line-height:1.65;color:#17131c}</style></head><body>${safePreviewHtml(html)}</body></html>`;

  if (Platform.OS === 'web') {
    return (
      // @ts-ignore - iframe is a valid web element via react-native-web
      <iframe srcDoc={doc} style={{ width: '100%', height: 420, border: 'none' }} title="Email preview" />
    );
  }

  // Lazy require so the native WebView module never touches the web bundle.
